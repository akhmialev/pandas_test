...
def create_choice_gym(telegram_id):
    ...
    for gym in gyms:
        if gym['status_gym'] == 'основной':
            title = f"{gym['gym']} {gym['status_gym']}"
            main_gym.append(InlineKeyboardButton(text=title, callback_data=f'ch_gym_{title}'))
        else:
            title = f"{gym['gym']} дополнительный"
            extra_gym.append(InlineKeyboardButton(text=title, callback_data=f'ch_gym_{title}'))
    extra_gym.append(InlineKeyboardButton(text='Изменить привязанные залы', callback_data=f'cho_gym'))
